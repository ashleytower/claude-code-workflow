...
### Required Team Roles
- **Implementer(s)**: `@frontend` / `@backend` / `general-purpose`
- **Code Reviewer**: `@code-reviewer` - reviews every changeset
- **Build Validator**: `@verify-app` - runs full CI (typecheck, test, lint, build)
- **Code Simplifier**: `@code-simplifier` - post-implementation cleanup

### TDD Rules
- New features: write failing test FIRST, then implement
- Bug fixes: ALWAYS write regression test that reproduces the bug first
...
### Parallel Agents
Use `dispatching-parallel-agents` skill when tasks are independent.

### Feedback Loops
If verification fails: fix -> re-verify -> loop until green. Never mark complete while failing.
